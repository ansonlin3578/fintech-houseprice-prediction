...
def preprocessing(data):
    ...
    # data = data[data.price.str.contains("aanvraag") == False].dropna()
    ...
    y = data.price
    X = data.drop('price', axis = 1).drop('status', axis = 1).drop('state', axis = 1).drop('sold_date', axis = 1)
...
model(pipe_ols, param_ols, X_train, y_train, X, y,"ols_tmp")
model(pipe_xgb, param_xgb, X_train, y_train, X, y,"xgb_tmp")
model(pipe_knn, param_knn, X_train, y_train, X, y,"knn_tmp")
model(pipe_lasso, param_lasso, X_train, y_train, X, y,"lasso_tmp")
model(pipe_ridge, param_ridge, X_train, y_train, X, y,"ridge_tmp")
model(pipe_poly, param_poly, X_train, y_train, X, y,"poly_tmp")
model(pipe_tree, param_tree, X_train, y_train, X, y,"tree_tmp")
model(pipe_forest, param_forest, X_train, y_train, X, y,"forest_tmp")
model(pipe_neural, param_neural, X_train, y_train, X, y,"neural_tmp")
